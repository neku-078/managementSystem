...
<?php
    ...
    if(isset($_GET['borrow_id'])) {
        ...
        $sql = "SELECT Club_ID FROM borrowing_list WHERE Borrowing_list_ID = ".$borrow_id.";";
        if($result = mysqli_query($conn, $sql)) {
            $club_id = mysqli_fetch_row($result)[0];
            $now_id = $_SESSION['uid'];
            if($_SESSION['uid'] != $club_id) {
                echo "<script>
                        alert('不可以偷看別人的借用資料！！');
                        location.href = './dashboard_club.php';
                    </script>";
            }
        }
        else {
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <?php
                        if($check_info) {
                            echo "<form class='borrow_list' action='sql/borrowing_club_sql.php?borrow_id=".$borrow_id."' method='post'>";
                        }
                    ?>
                        <div class="row">
                            <div class="col-lg-6">
                                ...
                                <div class="card shadow mb-4">
                                    ...
                                    <?php
                                        if(!$check_info) {
                                            echo "<form action='borrowing_club.php' method='post'>";
                                        }
                                    ?>
                                        <div class="input-group" style="height: 50px; display: flex;">
                                            <p style="align-self: center;">借用單位：</p>
                                            <?php 
                                            echo '<input type="text" class="form-control form-control-sm bg-light border-0 col-md-5" disabled="disabled" placeholder="'.$_SESSION['uid'].'" aria-label="Search" aria-describedby="basic-addon2">';
                                             ?>
                                        </div>
                                            ...
                                            <p>活動地點：
                                                ...
                                                <select id="place_select" name="event_space" class="custom-select custom-select-sm">
                                                    ...
                                                    <?php 
                                                        ...
                                                        while($space_data = mysqli_fetch_row($query)){
                                        ...
                                        <div>
                                            <div id="equipment_form">
                                                <p>
                                                    ...
                                                    &nbsp;
                                                    數量：
                                                    <select id="equipment_amount" name="equipment_amount" class="custom-select custom-select-sm col-md-2">
                                                    </select>
                                                    <button id="add_equipment" class="btn btn-secondary form-control-sm" type="button" disabled>
                                                        <span class="icon">
                                                            <i class="fas fa-arrow-right"></i>
                                                        </span>
                                                    </button>
                                                    <p id="borrowable_amout">可借數量：</p>
                                                    <script type="text/javascript">
                                                        function equipment_select_change() {
                                                            ...
                                                            document.getElementById("borrowable_amout").innerText = "可借數量：" + equipment_amount[equipment_id];
                            ...
                            <script type="text/javascript">
                                if(borrow_id != '0') {
                                    ...
                                    //不可編輯
                                    document.getElementById('borrow_date').disabled = true;
                                    document.getElementById('return_date').disabled = true;
                                    document.getElementById('place_select').disabled = true;
                                    document.getElementById('check_info').disabled = true;
                                    document.getElementById('check_info').style.display = 'none';
                                    document.getElementById('equipment_select').disabled = true;
                                    document.getElementById('equipment_amount').disabled = true;
                                    document.getElementById('add_equipment').disabled = true;
                                    ...
                                    for(var i = 0; i < equipment_data.length; i++) {
                                        ...
                                        if(borrow_id == '0') {
                                            tr.innerHTML="<tr>"+
                                                            "<td>"+equipment_data[i][0]+"</td>"+
                                                            "<td>"+equipment_data[i][1]+"</td>"+
                                                            "<td>"+equipment_data[i][2]+"</td>"+
                                                            "<td><a href='javascript:;' class='btn btn-sm btn-danger btn-icon-split'>"+
                                                            "<span class='icon'>"+
